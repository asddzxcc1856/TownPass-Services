...
const emits = defineEmits(['isAddScheduleState']);

const path = `/schedules/${props.akey}`;
</script>

<template>
  <button
    @click="
      if (props.isAddSchedule == false && route.query.attractionId) {
        emits('isAddScheduleState');
        if (btn_text == '新增行程') {
  ...
  >
    ...
    <img
      v-if="props.isAddSchedule == false && route.query.attractionId"
      class="w-12 min-w-0"
