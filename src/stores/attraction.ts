...
export const useAttractionStore = defineStore('attractions', () => {
  ...
  const recommand = (lat: number, lon: number, radius: number) => {
    ...
    for (let i = 0; i < attractions.length; i++) {
      ...
      let distance = 2 * Math.asin(Math.sqrt(Math.pow(Math.sin(lat_ / 2), 2) +
      Math.cos(Rad(lat)) * Math.cos(Rad(attractions[i].nlat)) * Math.pow(Math.sin(lon_ / 2), 2)));
      distance = distance * 6378.137;
      distance = Math.round(distance * 10000) / 10000;

      if (distance < radius) {
        Ids.push(attractions[i].id);
      }
    }
